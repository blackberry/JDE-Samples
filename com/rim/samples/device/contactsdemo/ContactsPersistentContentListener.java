 ...
 */
/* package */final class ContactsPersistentContentListener implements
     ...
     * because data is always encoded inside the contact models; thus, there is
     ...
     * are changed. Re-encodes the Contacts app's database accordingly (if one
     * exists).
     * 
     * @param generation
    ...
    public void persistentContentModeChanged(final int generation) {
        final PersistentObject persist =
                PersistentStore.getPersistentObject(0x8c746bdb8fb74512L); // com.rim.samples.device.contactsdemo
        ...
        synchronized (persist) {
            final Vector models = (Vector) persist.getContents();
            ...
            for (int i = 0; i < numModels; ++i) {
                final ContactModel model = (ContactModel) models.elementAt(i);
                model.reEncode();
                if (generation != PersistentContent.getModeGeneration()) {
                    // Device's Content Protection/Compression security settings
                    // have changed again
                    // since the listener was last notified. Abort this
                    // re-encoding because it will
                    // have to be done again anyway according to the new Content
            ...
            persist.commit(); // Commit the updated database to the persistent
                              // store.
        }
    }
}
